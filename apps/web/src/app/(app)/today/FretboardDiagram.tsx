...
export function FretboardDiagram(props: { spec: FretboardDiagramSpec }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await fetch("/api/diagram/fretboard", {
        method: "POST",
      ...
      if (!res.ok || !payload || (payload as any).ok === false) {
        setError((payload as any)?.error ?? "Failed to render diagram");
        return;
      }
